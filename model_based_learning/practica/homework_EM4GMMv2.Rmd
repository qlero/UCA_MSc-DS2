...
**Note**: *Going lower in terms of split (e.g. 0.66.../0.33..) starts yielding an increasing number of errors. This might be related to the splits not providing enough items to the train set to compute working parameters (e.g. means, variance, etc.) This has not been tested/resolved fully, however. R traceback was not very helpful. If an error occurs involving a wrongly-dimensioned matrix, re-knit/rerun. Clearing the environment in RStudio might also help*.
...
The following section is used to declare all of the R-markdown functions along with the implementations (from scratch) of the multivariate EM algorithm and the K-Means algorithm. Functions are declared in alphabetical order.
...
# 3 - Comparing EM and K-Means Algorithms

#### 3.1.1. Computing and plotting the results (Full Dataset case)

We compute the EM (with and without the K-Means initialization) and K-Means algorithms on the full dataset.
...
We display the final clustering via a single K-Means:
...
We display the final clustering via the EM algorithm (with and without KMeans):
...
We observe that the EM algorithm with a random initialization performs differently from the K-Means algorithm or its counterpart initialized with the K-Means. Furthermore, and at first glance, and given our knowledge of the dataset, the EM with random initialization perform better overall.

#### 3.1.2. Computing and plotting the results (Train-Test Split)

We compute the EM and K-Means algorithms on the training data.
...
We then cluster the test set using the results obtained with the train set.
...
Compared to using the full dataset, we find that, in general, the EM algorithm with K-Means initialization offers a clustering slightly closer to what we would expect. In the meantime, the clustering resulting from a simple K-Means is similar to that obtained on the full dataset.
...
In general, we observe that the EM algorithm performs better than the K-Means algorithm. 

Furthermore we observe that in general, a random initialization performs better than a k-means initialization for the EM algorithm in the present bivariate case. 

As the K-Means provides somewhat already optimized centroids, the algorithm does not seem to update the mixture parameters as much in the same amount of iterations as the random initialization. However, we still observe that the EM algorithm improves on the performance of the K-Means alone.

These observations are highlighted by our metrics (class error and adjusted rand index) where the EM algorithm obtains better results (lower class error and higher adjusted rand index) compared to the KM algorithm.
...
Starting with a cluster number of 3 (which we know as the number of types of wine in the dataset), we increase the cluster number and check with different heuristics/criteria in order to select the best EM model.

Given our previous results, we use the EM algorithm with a random initialization to compute our criteria.
...
We find that the Akaike Information Criterion is unstable compared to the Bayesian Information Criterion and the Cross-Validated Likelihood (here the double cross-validation). Furthermore, the AIC consistently prefers models with a higher number of clusters.

Meanwhile the latter two criteria consistently yield 3 as the most optimal number of clusters (on the whole dataset for the BIC and the case of the train/test split for the double cross-validation).

As expected, the BIC penalizes numbers of clusters higher than necessary more than the AIC, yielding a result closer (here equaling) the ground truth.

# 5 - Testing with higher-dimensional spaces

We will operate in two steps. At first we will hand-pick a select few features to observe whether we obtain any change compared to the previous bivariate case. Then we sample at random features sets and iterate over them.

#### 5.1. Hand-selected features

We selected the first 3 features in this first hand-selected testing of the EM algorithm. This corresponds to selecting the variables 2, 3, and 4 of the Wine dataset.
...
We compute the best cluster number for the whole dataset using the BIC criterion.
...
As we find via the BIC that the best results are again obtained using 3 clusters with this selected high-dimensional dataset, we try to apply the EM algorithm on a train split of the dataset. Then, we will check the resulting clustering on the test split.
...
```
Once done with the EM iteration, we visualize the resulting clustering on the test set by focusing on the two features we previously focused on with the bivariate case:
...
Going through multiple trials of the present EM algorithm, we find similar results as with the bivariate case, that is: the most efficient number of clusters is 3. This result is expected as we know there are 3 types of wine in the dataset. Having a proof through the application of the EM algorithm is reassuring.

#### 5.2.1. Randomly selected features

As previously mentioned, we also test a number of different higher-dimension sets of features. To do so, we randomly generate five 10-element permutations and five 5-element permutations from the dataset (out of 27 feature columns). From the latter set of 5-element permutations, we build the corresponding sets of 3-element, and 4-element sub-permutations.

As such, the selection of permutations would look something like:

- (a,b,c,d,e), (a,b,c,d), (a,b,c,e), (a,c,d,e), ..., (b,c,d), (c,d,e)

- i.e. 5 10-permutations, 5 5-permutations, 600 4-permutations, 14400 3-permutations.
...
We remove the duplicated permutations if there is any:
...
#### 5.2.2. Computing the AIC and BIC for the 10-element permutations
...
As previously seen, we see that the AIC yields higher and different clusters per given set of permutated features. However, it seems that the BIC also starts yielding a higher number of clusters as a result.

We might want to hypothesize that the EM algorithm is less efficient in higher dimensions as we are trying to split a population of 3 types using an increasingly more complex space.

#### 5.2.3. Computing the AIC and BIC for the 5-element permutations
...
As previously seen, we see that the AIC yields higher and different clusters per given sets of permutated features. As with the case of the 10-element permutations, we observe that depending on the permutations, the BIC also yields a higher number of clusters. However, given a run of 5-element permutations, we tend to find an average lower number of clusters than with 10-element permutations.

Here, we might refine our hypothesis again: that the EM algorithm is indeed less efficient in higher dimensions as we are trying to split a population of 3 types using an increasingly more complex space. As such, the lower the dimension of the data, the easier it is to cluster the Wine dataset.

Of note, this lack of performance at a higher dimension seems to lso be highly dependent on the selected features.

#### 5.2.4. Computing the BIC for the 4-element permutations
...
Compared to the 10-element and 5-element permutations, and only using the BIC criterion, we find that most of the runs yield 3 as the best number of clusters. This might reinforce our hypothesis.

#### 5.2.5. Computing the BIC for the 3-element permutations
...
In most run, we always yield 3 as the best number of clusters.

#### 5.2.6. Overall observations

Overall, it seems that the higher the dimension we decide to iterate over with the EM algorithm, the higher the resulting number of clusters we find (even when using the BIC criterion which penalizes a more complex model).

Though at this stage it remains an hypothesis, we can suppose that the EM algorithm, which finds a local optimum might get stuck in such a situation in a higher dimensional space. This may yield non-optimum results in higher dimensions.

As such, and given these experimental results on the Wine dataset (and granted that we know the actual number of wine types in the dataset), we might want to inform our heuristic by trying to keep our selection of features low. 
