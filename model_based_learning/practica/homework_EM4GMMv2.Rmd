...
- **Error** -- Assymetric matrix error:

> This error may occurs on ``computeBIC`` or ``expectationMaximization`` calls for instance
>
> The functions may crash due to the randomized aspect of the algorithm process resulting in an asymmetric matrix (This happens when a cluster is computed to only contain a single datapoint)
> 
> <u>Error message</u>: (max(abs(sigma - t(sigma))) > sqrt(.Machine$double.eps)) stop("sigma must be a symmetric matrix") : missing value where TRUE/FALSE needed

- **Error** -- EM random initialization fail:

> This error occurs on ``expectationMaximization``
>
> Sometimes the random initialization of the ``expectationMaximization`` function fails
> 
> <u>Error message</u>: Error in FUN(X[[i]], ...) : 'dimnames' applied to non-array

- **Error** -- Crash whe editing the code while running a chunk
...
>
> <u>Error message</u>: Error in FUN(X[[i]], ...) : object[[i]] not found

- **Warning** -- Overwritten methods by conflicting packages:

> This warning may occur on the ``computeAIC`` and ``computeBIC`` calls for instance
> 
> This is caused when multiple packages have the same named function.
>
> <u>Warning message</u>: 
...
We observe that the EM algorithm with a random initialization performs differently from the K-Means algorithm or its counterpart initialized with the K-Means. At first glance, and given our knowledge of the dataset, it seems that the EM with random initialization perform better overall. We will confirm this in the latter sections.
