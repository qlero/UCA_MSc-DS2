...
n = dim(X)[1]
min_n_clusters = length(unique(y))
max_n_clusters = 10
...
logLikelihood <- function(n, clusters, x, prop, mu, sigma) {
  ...
  # Computes the gammas per clusters
    ...
    for (cl in 1:clusters){
      lgn[,cl] = log(prop[cl]) + 
        matrix(dmvnorm(x, mu[cl,], sigma[[cl]], log=TRUE)) # remove matrix?
    }
...
mostMatchingDistribution <- function(clusters, x, prop, mu, sigma){
  ...
  clustering = findCluster(likelihoods)
...
findCluster <- function(likelihoods) {
  ...
  clustering = (likelihoods == apply(likelihoods, 1, max))
  clustering = apply(clustering, 1, which)
  
  # Return statement
  return(clustering)
}

expectationMaximization <- function(x, clusters, maxIterations=200) {
  ######
  ...
  ### TO CHECK WITH https://stackoverflow.com/questions/49834894/kmeans-results-as-initial-parameters-in-mclust
  ...
  for (loop in 1:maxIterations){
    ...
    #### Expectation step (with normalization step + logexp trick)
    LLH = logLikelihood(n, clusters, x, prop, mu, sigma)
    sum_LLH = sum(apply(LLH, 1, logSumExp))
    if (round(sum_LLH,4) <= round(previous_LLH,4)) {
      counter = counter + 1
      if (counter >= 5) {
          cat("\nEarly stopping at iteration ", loop, ".")
          break
        }
    } else {
      counter = 0
      previous_LLH = sum_LLH
    }
    #cat("\nStep ", loop, " LLH: ", sum_LLH)
    history_LLH = append(history_LLH, sum_LLH)
    
    gam = LLH - apply(LLH, 1, logSumExp)
    gam = exp(gam)
    ...
    for (cl in 1:clusters) {
      ...
      sigma[[cl]] =
        Reduce("+",lapply(1:n,function(i) 
          gam[i,cl] * (x[i,]-mu[cl,]) %*% t(x[i,]-mu[cl,])))/nk
    }
  }
  ...
  cat("\nEnd normalized log-likelihood: ", llh_sum, "\n")
  ...
  ret = list(
    ...
    "llh" = llh_results$likelihoods,
    "llh_convergence" = history_LLH,
    "llh_sum" = llh_sum
    )
  return(ret)
}
...
kMeans <- function(x, clusters, maxIterations=200) {
  ######
...
computeAIC <- function(x, max_cluster=max_n_clusters, print_steps=TRUE){
  ...
  for (j in min_n_clusters:max_cluster){
    ...
    akaike_value = akaikeIC(res$llh_sum, j, dim(x)[2])
...
computeBIC <- function(x, max_cluster=max_n_clusters, print_steps=TRUE){
  ...
  for (j in min_n_clusters:max_cluster){
    res = expectationMaximization(x, clusters=j)
    bayesian_value = bayesianIC(res$llh_sum, j, dim(x)[1], dim(x)[2])
...
```{r}
plot(X, col=resultsEM$clustering+1, pch=19)
points(resultsEM$means, col=1, pch=10)
```
...
test_resultsEM = mostMatchingDistribution(
  n_clusters, 
  X_test, 
  resultsEM_train$prop, 
  resultsEM_train$means, 
  resultsEM_train$Sigma
  )
...
```{r}
plot(X_test, col=test_resultsEM$clustering+1, pch=19)
points(resultsEM_train$means, col=1, pch=10)
```
...
```{r}
classError(resultsEM$clustering, y)
```
...
```{r}
classError(test_resultsEM$clustering, y_test)
```
...
```{r}
adjustedRandIndex(resultsEM$clustering, y)
```
...
```{r}
adjustedRandIndex(test_resultsEM$clustering, y_test)
```
...
Starting with a cluster number of 3, we increase the number and check with different heuristics and criteria to select our best EM model.
...
```{r}
plot(Xhd_test[,c(1,3)], col=test_resultsEMhd$clustering+1, pch=19); points(resultsEMhd_train$means[,c(1,3)], col=1, pch=10)
```
...
#### 5.2.3. Computing the AIC for the 4-permutations
...
#### 5.2.4. Computing the AIC for the 3-permutations
