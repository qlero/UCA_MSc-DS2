...
expectationMaximization <- function(x, clusters, maxIterations=200, kmInit = F) {
  ######
  ...
  for (loop in 1:maxIterations){
    ...
    if (round(sum_LLH,4) <= round(previous_LLH,4)) {
      counter = counter + 1
      if (counter >= 5) {cat("\nEarly stopping at loop:", loop); break}
...
computeAIC <- function(x, max_cluster=max_n_clusters, print_steps=TRUE){
  ...
  for (cl in min_n_clusters:max_cluster){
    EM = expectationMaximization(x, clusters=cl)
...
doubleCrossValidation <- function(x_train, x_test, folds=10, max_cl=max_n_clusters) {
  ######
  ...
  for (cl in min_n_clusters:max_cl){
    ...
    for (kFold in fold_indexes){
      ...
      EM = expectationMaximization(x_train_train, cl)
...
resultsEMwithoutKMinit = expectationMaximization(X, n_clusters)

```
```{r}

resultsEMwithKMinit = expectationMaximization(X, n_clusters, kmInit=T)
...
plot(X, col=resultsEMwithoutKMinit$clustering+1, pch=19)
points(resultsEMwithoutKMinit$means, col=1, pch=10)
...
plot(X, col=resultsEMwithKMinit$clustering+1, pch=19)
points(resultsEMwithKMinit$means, col=1, pch=10)
